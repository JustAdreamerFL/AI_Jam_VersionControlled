...
```powershell
C:\Users\student\.conda\envs\mlagents\python.exe -m pip install torch==2.0.1+cu117 torchvision==0.15.2+cu117 --index-url https://download.pytorch.org/whl/cu117
```

### 5. Nainštaluj ML-Agents z lokálneho repo (editable mode)
```powershell
# Najprv ml-agents-envs
C:\Users\student\.conda\envs\mlagents\python.exe -m pip install -e "D:\uniza_ai_jam\AI_Jam_VersionControlled\ml-agents\ml-agents-envs"

# Potom hlavný ml-agents balík
C:\Users\student\.conda\envs\mlagents\python.exe -m pip install -e "D:\uniza_ai_jam\AI_Jam_VersionControlled\ml-agents\ml-agents"
```

### 6. Fix PyTorch verzie (ak sa nainštalovala zlá)
```powershell
C:\Users\student\.conda\envs\mlagents\python.exe -m pip install torch==2.0.1+cu117 --index-url https://download.pytorch.org/whl/cu117 --force-reinstall --no-deps
```
...
### Spustenie tréningu
```powershell
# Vždy najprv nastav env variables kvôli R: disk permissions
$env:PIP_CACHE_DIR = "C:\Users\student\.cache\pip"

# Potom spusti ML-Agents
C:\Users\student\.conda\envs\mlagents\Scripts\mlagents-learn.exe config/your_config.yaml --run-id=MyTraining

# Alebo cez Python modul:
C:\Users\student\.conda\envs\mlagents\python.exe -m mlagents.trainers.learn config/your_config.yaml --run-id=MyTraining
```
